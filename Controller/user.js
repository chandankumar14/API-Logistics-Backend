  ...
  User.find({ email: email, password: password })
    .then((result) => {
      if (result.length >= 1) {
        ...
        res.status(200).json({
          message: "User Not LoggedIn Sucessfully",
...
exports.signUp = (req, res, next) => {
  ...
  SignInUser.save()
    .then((result) => {
      res
        .status(200)
        .json({ message: "User SignedUp Sucessfully", user: result });
    })
    .catch((err) => {
      res.status(500).json({ message: err });
    });
...
exports.deleteItem = (req, res, next) => {
  ...
  User.remove({ _id: id })
    .then((result) => {
      res
        .status(200)
        .json({ message: "User deleted Sucessfully", user: result });
    })
    .catch((err) => {
      res.status(500).json({ message: err });
    });
...
exports.googleLogin = (req, res, next) => {
  ...
  client
    ...
    .then((response) => {
      ...
      console.log(email_verified, given_name, email);
      if (email_verified) {
